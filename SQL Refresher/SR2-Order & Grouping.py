...
# MAGIC %sql
# MAGIC -- Let's caculate number of employees for each club
# MAGIC SELECT club_id, COUNT(1) AS total_members
# MAGIC FROM employee
# MAGIC GROUP BY club_id

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Let's caculate number of employees for each club and sort the result in DECREASING order of total_members
# MAGIC SELECT club_id, COUNT(1) AS total_members
# MAGIC FROM employee
# MAGIC GROUP BY club_id
# MAGIC ORDER BY total_members DESC

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Let's caculate number of employees for each club and sort the result in INCREASING order of total_members
# MAGIC SELECT club_id, COUNT(1) AS total_members
# MAGIC FROM employee
# MAGIC GROUP BY club_id
...
# MAGIC %sql
# MAGIC SELECT C.club_name, COUNT(1) AS total_members
# MAGIC FROM employee E
# MAGIC INNER JOIN club C
# MAGIC ON E.club_id = C.club_id
# MAGIC GROUP BY C.club_name
# MAGIC HAVING total_members > 1
# MAGIC ORDER BY total_members DESC
...
# MAGIC %sql
# MAGIC SELECT D.dept_name AS Department, C.club_name AS Club, COUNT(1) AS total_members
# MAGIC FROM employee E
# MAGIC INNER JOIN club C 
# MAGIC ON E.club_id = C.club_id
# MAGIC INNER JOIN department D
# MAGIC ON E.dept_id = D.dept_id
# MAGIC GROUP BY D.dept_name, C.club_name
# MAGIC ORDER BY total_members 
# MAGIC 
# MAGIC -- so now you can see, there are multiple rows for marketing & IT. As these two department has members from different club.
...
# MAGIC %sql
# MAGIC SELECT * FROM employee order by empcode

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT D.dept_name, ROUND(AVG(ES.basic_salary), 2) AS AVG_BASIC_SALARY
# MAGIC FROM
# MAGIC employee E INNER JOIN emp_salary ES
# MAGIC ON E.empcode = ES.empcode
# MAGIC INNER JOIN department D
# MAGIC ON E.dept_id = D.dept_id
# MAGIC GROUP BY D.dept_name
# MAGIC ORDER BY AVG_BASIC_SALARY
...
# MAGIC %sql
# MAGIC SELECT D.dept_name, ROUND(SUM(ES.basic_salary), 2) AS TOTAL_BASIC_SALARY
# MAGIC FROM
# MAGIC employee E INNER JOIN emp_salary ES
# MAGIC ON E.empcode = ES.empcode
# MAGIC INNER JOIN department D
# MAGIC ON E.dept_id = D.dept_id
# MAGIC GROUP BY D.dept_name
# MAGIC ORDER BY TOTAL_BASIC_SALARY
