...
# MAGIC %sql
# MAGIC -- let's create one CTE which will calculate department wise salary. We will use full join so that we can include department which does not exists, this will generate NULL values for AVG salary field. In the second expression we will remove those invalid rows (basic salary with null values). This can be done in a simpler way using a single query but for the sake of tutorial, I am using 2 different CTE to achevie this task, but in real life scenario CTE is not a good way to acheive this. You can simple do this by one group statement & filter (WHERE). You can see in the below query we applied outer join (just for the sake of tutorial, do not think about the business logic here). You can see many invalid rows with dept_name and AVG_BASIC_SALARY as null. Now we will change this query to CTE & using another CTE we will clean these rows.
# MAGIC --WITH cte_dept_salary AS(
# MAGIC SELECT D.dept_name, AVG(ES.basic_salary) AS AVG_BASIC_SALARY
# MAGIC FROM employee E FULL JOIN 
# MAGIC emp_salary ES ON E.empcode = ES.empcode
# MAGIC FULL JOIN
# MAGIC department D ON E.dept_id = D.dept_id
# MAGIC GROUP BY D.dept_name
# MAGIC --)
...
# MAGIC   SELECT * FROM cte_avg_salary 
# MAGIC   WHERE dept_name IS NOT NULL AND AVG_BASIC_SALARY IS NOT NULL
# MAGIC )
# MAGIC 
# MAGIC SELECT * FROM cte_avg_salary_clean;
...
# MAGIC %sql 
# MAGIC -- We can use CTE with views also. Let's use above nested cte in a view
# MAGIC CREATE OR REPLACE VIEW VW_DEPT_SALARY AS
# MAGIC WITH cte_avg_salary AS (
# MAGIC     SELECT D.dept_name, AVG(ES.basic_salary) AS AVG_BASIC_SALARY
# MAGIC FROM employee E FULL JOIN 
# MAGIC emp_salary ES ON E.empcode = ES.empcode
...
# MAGIC ),
# MAGIC cte_avg_salary_clean AS (
# MAGIC   SELECT dept_name AS Department, ROUND(AVG_BASIC_SALARY, 2) AS AVG_BASIC_SALARY FROM cte_avg_salary 
# MAGIC   WHERE dept_name IS NOT NULL AND AVG_BASIC_SALARY IS NOT NULL
# MAGIC )
# MAGIC SELECT * FROM cte_avg_salary_clean;
...
# MAGIC %sql
# MAGIC SELECT * FROM VW_DEPT_SALARY;
